...
client.once("clientReady", async () => {
  ...
  try{
    ...
    await voce.send({content:`🟢 **03 CRIADOR INICIADO COM SUCESSO**`});
    await voce.send({embeds:[new EmbedBuilder().setColor('#06b6d4').setTitle(`📋 COMANDOS - 03 CRIADOR`).setDescription("!cria / !criador - Criador de bots/sistemas
!ping - Testa conexao
!hub - Hub geral
!veri - Verificacao
!ia - Inteligencia
...
process.on('unhandledRejection', e => console.log(`🔴 03 CRIADOR: ${e}`));

if(!TOKEN_USAR || TOKEN_USAR.length < 20) {
  console.error(`❌ VARIAVEL ${TOKEN_CRIADOR} NAO ENCONTRADA OU VAZIA`);
  process.exit(1);
}
