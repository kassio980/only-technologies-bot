...
const TOKEN_USAR = process.env.TOKEN_HUB || '';
...
client.on('clientReady', async () => {
  ...
  try{
    ...
    await voce.send({embeds:[new EmbedBuilder().setColor('#22c55e').setTitle(`🟢 10 HUB IA INICIADO`).setDescription('Todos comandos funcionando ✅')]});
  }catch{}
});

client.on('error', e => console.log(`🔴 10 HUB IA ERRO: ${e.message}`));
process.on('unhandledRejection', e => console.log(`🔴 10 HUB IA: ${e.message}`));

if(!TOKEN_USAR || TOKEN_USAR.length < 40) {
  console.error(`❌ ERRO: Variável ${TOKEN_HUB} não encontrada ou vazia`);
  process.exit(1);
}
