...
const TOKEN_USAR = process.env.TOKEN_PAINEL || '';
...
client.once("clientReady", async () => {
  ...
  console.log(`🟢 05 PAINEL | ONLINE E RESPONDENDO`);
  ...
  try{
    ...
    await voce.send({content:`🟢 **05 PAINEL INICIADO COM SUCESSO**`});
    await voce.send({embeds:[new EmbedBuilder().setColor('#a855f7').setTitle(`📋 COMANDOS - 05 PAINEL`).setDescription("!painel / !cpainel - Acessar painel de controle
!ping - Testa conexao
!hub - Hub geral
!veri - Verificacao
!ia - Inteligencia
...
client.on('error', e => console.log(`🔴 05 PAINEL ERRO: ${e.message}`));
process.on('unhandledRejection', e => console.log(`🔴 05 PAINEL: ${e}`));

if(!TOKEN_USAR || TOKEN_USAR.length < 20) {
  console.error(`❌ VARIAVEL ${TOKEN_PAINEL} NAO ENCONTRADA OU VAZIA`);
  process.exit(1);
}
