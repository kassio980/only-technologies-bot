...
client.on('clientReady', async () => {
  ...
  try{
    ...
    await voce.send({embeds:[new EmbedBuilder().setColor('#22c55e').setTitle(`🟢 05 PAINEL INICIADO`).setDescription('Todos comandos funcionando ✅')]});
  }catch{}
});
...
if(!TOKEN_USAR || TOKEN_USAR.length < 40) {
  console.error(`❌ ERRO: Variável ${TOKEN_PAINEL} não encontrada ou vazia`);
  process.exit(1);
}
