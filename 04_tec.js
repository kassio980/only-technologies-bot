...
const TOKEN_FINAL = process.env.TOKEN_TECNICO || cfg?.B?.CL?.t || cfg?.B?.VR?.t || cfg?.B?.CB?.t || cfg?.B?.TC?.t || cfg?.B?.PN?.t || cfg?.B?.SG?.t || cfg?.B?.TK?.t || cfg?.B?.AD?.t || cfg?.B?.CT?.t || cfg?.B?.HB?.t || '';
...
client.on('clientReady',async()=>{
  console.log(`🟢 04 TÉCNICO | ONLINE`);
  client.user.setPresence({status:'online'});
  client.user.setActivity({name:'ONLY TECHNOLOGIES',type:ActivityType.Watching});
  try{const d=await client.users.fetch(DONO_ID);await d.send({embeds:[{color:0x22c55e,title:`🟢 04 TÉCNICO INICIADO`,timestamp:new Date()}]})}catch{}
})
client.on('error',e=>console.log(`🔴 04 TÉCNICO: ${e.message}`))
process.on('unhandledRejection',e=>console.log(`🔴 04 TÉCNICO: ${e.message}`))
if(!TOKEN_FINAL){console.error(`❌ FALTA VARIÁVEL: TOKEN_TECNICO`);process.exit(1)}
