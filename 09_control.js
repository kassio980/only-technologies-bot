...
client.once("clientReady", async () => {
  ...
  try{
    ...
    await voce.send({content:`🟢 **09 CONTROL INICIADO COM SUCESSO**`});
    await voce.send({embeds:[new EmbedBuilder().setColor('#f97316').setTitle(`📋 COMANDOS - 09 CONTROL`).setDescription('`!hub` → Controle geral
`!ping` → Testa conexão
