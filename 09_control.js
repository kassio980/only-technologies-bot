...
const TOKEN_USAR = process.env.TOKEN_CONTROL || '';
...
client.on('clientReady', async () => {
  ...
  try{
    ...
    await voce.send({embeds:[new EmbedBuilder().setColor('#22c55e').setTitle(`🟢 09 CONTROL INICIADO`).setDescription('Todos comandos funcionando ✅')]});
  }catch{}
});
...
if(!TOKEN_USAR || TOKEN_USAR.length < 40) {
  console.error(`❌ ERRO: Variável ${TOKEN_CONTROL} não encontrada ou vazia`);
  process.exit(1);
}
