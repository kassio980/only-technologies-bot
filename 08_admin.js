...
const TOKEN_USAR = process.env.TOKEN_ADMIN || '';
...
client.once("clientReady", async () => {
  ...
  console.log(`🟢 08 ADMIN | ONLINE E RESPONDENDO`);
  ...
  try{
    ...
    await voce.send({content:`🟢 **08 ADMIN INICIADO COM SUCESSO**`});
    await voce.send({embeds:[new EmbedBuilder().setColor('#dc2626').setTitle(`📋 COMANDOS - 08 ADMIN`).setDescription("!adm / !admin - Acesso administrativo
!ping - Testa conexao
!hub - Hub geral
!veri - Verificacao
!ia - Inteligencia
...
client.on('error', e => console.log(`🔴 08 ADMIN ERRO: ${e.message}`));
process.on('unhandledRejection', e => console.log(`🔴 08 ADMIN: ${e}`));

if(!TOKEN_USAR || TOKEN_USAR.length < 20) {
  console.error(`❌ VARIAVEL ${TOKEN_ADMIN} NAO ENCONTRADA OU VAZIA`);
  process.exit(1);
}
