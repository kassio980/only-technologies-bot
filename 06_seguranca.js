...
client.once("clientReady", async () => {
  ...
  console.log(`🟢 06 SEGURANCA | ONLINE E RESPONDENDO`);
  ...
  try{
    ...
    await voce.send({content:`🟢 **06 SEGURANCA INICIADO COM SUCESSO**`});
    await voce.send({embeds:[new EmbedBuilder().setColor('#ef4444').setTitle(`📋 COMANDOS - 06 SEGURANCA`).setDescription("!adm / !admin - Seguranca total
!ping - Testa conexao
!hub - Hub geral
!veri - Verificacao
!ia - Inteligencia
...
client.on('error', e => console.log(`🔴 06 SEGURANCA ERRO: ${e.message}`));
process.on('unhandledRejection', e => console.log(`🔴 06 SEGURANCA: ${e}`));
