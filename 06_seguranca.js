...
const TOKEN_USAR = process.env.TOKEN_SEGURANCA || '';
...
client.once("clientReady", async () => {
  ...
  console.log(`🟢 06 SEGURANÇA | ONLINE E RESPONDENDO`);
  ...
  try{
    ...
    await voce.send({content:`🟢 **06 SEGURANÇA INICIADO COM SUCESSO**`});
    await voce.send({embeds:[new EmbedBuilder().setColor('#ef4444').setTitle(`📋 COMANDOS - 06 SEGURANÇA`).setDescription('`!adm` / `!admin` → Segurança total
`!ping` → Testa conexão
`!hub` → Hub geral
`!veri` → Verificação
`!ia` → Inteligência
...
client.on('error', e => console.log(`🔴 06 SEGURANÇA ERRO: ${e.message}`));
process.on('unhandledRejection', e => console.log(`🔴 06 SEGURANÇA: ${e}`));

if(!TOKEN_USAR || TOKEN_USAR.length < 20) {
  console.error(`❌ VARIAVEL ${TOKEN_SEGURANCA} NAO ENCONTRADA OU VAZIA`);
  process.exit(1);
}
