...
client.on('clientReady', async () => {
  console.log(`🟢 06 SEGURANCA | ONLINE`);
  client.user.setPresence({status:'online'});
  client.user.setActivity({name:'ONLY TECHNOLOGIES', type: ActivityType.Watching});
  try{ await (await client.users.fetch(DONO_ID)).send(`🟢 06 SEGURANCA INICIADO`); }catch{}
});
...
if(!TOKEN_USAR || TOKEN_USAR.length < 20) {
  console.error(`❌ VARIAVEL ${TOKEN_SEGURANCA} NAO ENCONTRADA`);
  process.exit(1);
}
